...
wget -q -O /tmp/helios-consul.deb 'https://github.com/SVT/helios-consul/releases/download/0.26/helios-consul_0.26_all.deb' && \
dpkg --force-confdef --force-confold -i /tmp/helios-consul.deb ;

# Setup helios agent to use helios-consul
echo 'ENABLED=true \
HELIOS_AGENT_OPTS="--service-registry http://127.0.0.1:8500 --service-registrar-plugin /usr/share/helios/lib/plugins/helios-consul-*.jar"' > /etc/default/helios-agent
